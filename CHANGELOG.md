...
- Typo in `setuptools` specification of packages generated by the
  Playground.
- Non-thread-safe methods of loading external databases in the
  `SQLObject` and `variables snapshot db` features.
- Issue with converting Markdown to PDF when the Markdown includes
  tables, with the latest version of Pandoc.
...
### Fixed
- Downgraded `numpy` for compatibility with Python 3.10.
